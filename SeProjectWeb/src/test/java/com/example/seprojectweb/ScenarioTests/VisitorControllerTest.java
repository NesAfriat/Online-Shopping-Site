...
class VisitorControllerTest {
    ...
    static void beforeAll() {
        ...
        try {
            ...
            visitorController.register(visitor.getId(), "dubi", "gal");
    ...
    void findMember() {
        try{
            Member member = (Member) PersistenceManager.find(Member.class, "tsvika");
            assertEquals("tsvika", member.getUsername());
        } catch (Exception e) {
            e.printStackTrace();
            fail();
    ...
    void findAllMembers() {
        try{
            List members = PersistenceManager.findAll(Member.class);
            List<String> userNames = new LinkedList<>();
            for (Object member : members){
                Member realMember = (Member) member;
                userNames.add(realMember.getUsername());
            }
            assertTrue(userNames.contains("tsvika") && userNames.contains("dubi"));
        } catch (Exception e) {
            e.printStackTrace();
            fail();
        }
    ...
    void changePassword() {
        try{
            Member member = visitorController.changePassword("tsvika","hadar");
            assertEquals("tsvika", member.getUsername());
        } catch (Exception e) {
            e.printStackTrace();
            fail();
    ...
    void cartSaveForReLoginMember() {
        ...
        try {
            ...
            Product product = shop.addProduct("bike",123,"bike",1,"bike",dbConnection);
