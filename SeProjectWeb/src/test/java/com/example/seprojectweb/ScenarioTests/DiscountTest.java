...
public class DiscountTest {
    ...
    void setUp() throws InnerLogicException {
        ...
        productId1 = shop.addProduct("item1",25.5,"des item1",4,"cat1",dbConnection).getId();
        productId2 = shop.addProduct("item2",22,"des item2",44,"cat2",dbConnection).getId();
        productId3 = shop.addProduct("item3",22,"des item3",41,"cat3",dbConnection).getId();
    ...
    void addProductDiscount(double percentage, Date lastValidDate, int productId) throws InnerLogicException {
        ...
        shop.addProductDiscount(percentage+0.1, lastValidDate, productId);
        //shop.addProductDiscount(percentage, lastValidDate, productId);
...
//                Arguments.of(0.3, future,productId2),
    ...
    @DisplayName("add new discount => Success")
    @ParameterizedTest(name = "{index} => percentage = {0}, lastValidDate = {1}, productId = {2}")
    @MethodSource("pProviderXoR")
    void addXoRProductDiscount(double percentage, Date lastValidDate, int productId) throws InnerLogicException {
        int quantity = 2;
        Product product = shop.getProduct(productId);
        int id1 = shop.addProductDiscount(percentage, lastValidDate, productId).getId();
        int id2 = shop.addProductDiscount(percentage-0.1, lastValidDate, productId).getId();
        shop.addXorDiscount(id1, id2);
        ShoppingBasket shoppingBasket = new ShoppingBasket();
        shoppingBasket.addProduct(product, quantity);
        shop.applyDiscounts(shoppingBasket);
        double basketPrice = shoppingBasket.getTotalPrice();
        assertEquals(product.getPrice() * quantity * (1-percentage+0.1), basketPrice);
    }
    private static Stream<Arguments> pProviderXoR() {
        Date today = new Date();
        Date future = new Date(today.getTime() + 100000000);
        return Stream.of(
//                Arguments.of(0.2, future,productId1),
//                Arguments.of(0.3, future,productId2),
    ...
    void andDiscount(){
        ...
        try{
            ...
            int discountId = shop.addCategoryDiscount(percentage, future, "cat1").getId();
