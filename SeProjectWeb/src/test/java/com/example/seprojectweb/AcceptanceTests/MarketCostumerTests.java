...
public class MarketCostumerTests extends MarketTests{
    ...
    void purchaseShoppingCartIllegalQuantity() {
        ...
        Response<List<PurchaseHistoryResponse>> response= purchaseShoppingCart(visitor4.getId(),creditCard,date,cvs,country,city,street,zip);
        assertFalse(response.isErrorOccurred());
    ...
    void purchaseShoppingCartNotVisitor() {
        Response<List<PurchaseHistoryResponse>> response= purchaseShoppingCart(notUserId,creditCard,date,cvs,country,city,street,zip);
        assertTrue(response.isErrorOccurred());
        assertFalse(response.getErrorMessage().contains("fatal"));
        assertNull(response.getValue());
    }
    ...
    void purchaseShoppingCartIllegalCreditCard() {
        addProductToShoppingCart(visitor4.getId(),shopResponse.getId(),productResponse1.getId(),quantity);
        Response<List<PurchaseHistoryResponse>> response= purchaseShoppingCart(visitor4.getId(),invalidCreditNumber,date,cvs,country,city,street,zip);
        assertTrue(response.isErrorOccurred());
        assertFalse(response.getErrorMessage().contains("fatal"));
        assertNull(response.getValue());
    }
