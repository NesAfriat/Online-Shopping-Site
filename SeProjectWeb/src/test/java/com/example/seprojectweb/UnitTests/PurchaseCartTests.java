...
public class PurchaseCartTests {
    ...
    void setUp(){
        ...
        ConcurrentHashMap<Product, QuantityDiscount> productsMap = new ConcurrentHashMap<>();
        productsMap.put(productMock, new QuantityDiscount(1, 0.));
        when(shoppingBasketMock.getProducts()).thenReturn(productsMap.entrySet());
        when(shopMock.isOpen()).thenReturn(true);
        when(proxyDeliveryMock.bookDelivery("visitor","Israel","Ashdod","Malahim","123")).thenReturn(1);
        when(proxyPaymentMock.chargeCreditCard("123456789","11/26","visitor","123",productMock.getPrice()*productsMap.get(productMock).getDiscount())).thenReturn(1);
        ...
        try{
            ...
            when(shopControllerMock.getShop(1,1)).thenReturn(shopMock);
        }catch(Exception e){
            // do nothing
        }
    }
    ...
    void purchaseCartSuccess1(){
        ...
        try{
            ...
            // Act
            List<PurchaseHistory> res = purchaseCashier.purchaseCart(visitorMock.getId(),"123456789","11/26","123","Israel","Ashdod","Malahim","123");
    ...
    void purchaseCartFail1(){
        ...
        try{
            ...
            // Act
            List<PurchaseHistory> res = purchaseCashier.purchaseCart(visitorMock.getId(),"123456789A","11/26","123","Israel","Ashdod","Malahim","123");
    ...
    void purchaseCartFail3(){
        ...
        try{
            ...
            ConcurrentHashMap<Product, QuantityDiscount> productsMap = new ConcurrentHashMap<>();
            productsMap.put(productMock, new QuantityDiscount(5, 0.));
            when(shoppingBasketMock.getProducts()).thenReturn(productsMap.entrySet());
    ...
    void purchaseCartFail5(){
        ...
        try{
            ...
            productsMap.put(productMock, new QuantityDiscount(1, 0.));
            when(shoppingBasketMock.getProducts()).thenReturn(productsMap.entrySet());
            when(proxyPaymentMock.chargeCreditCard("123456789","11/26","visitor","123",productMock.getPrice()*productsMap.get(productMock).getQuantity())).thenReturn(-1);
    ...
    void purchaseCartFail6(){
        ...
        try{
            // Arrange
            when(shopMock.isOpen()).thenReturn(false);
